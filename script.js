...
function exportTop5OnlyPdf() {
  // Landscape for readability (many columns), with a second page for enablers/risks.
  ...
  const items = computeTopScenarioItems(metric, feasibleOnly, selectedOnly, 5);
  if (!items.length) {
    showToast("No saved scenarios available for Top 5 export.", "warning");
    return;
  }

  const topScenarios = items.map((it) => it.s);
  ...
  doc.setFont("helvetica", "bold");
  doc.setFontSize(18);
  doc.text("STEPS — Top 5 options (decision-maker mini export)", margin, y);
  y += 10;

  doc.setFont("helvetica", "normal");
  doc.setFontSize(10);
  const subtitle = `Ranked by: ${metricLabel(metric)}${feasibleOnly ? " · feasible only" : ""}${selectedOnly ? " · shortlisted only" : ""}`;
  ...
  const rows = topScenarios.map((s, i) => {
    const cap = s.capacity || computeCapacity(s.config);
    const mentorsReq = cap ? cap.totalMentorsRequired : 0;
    const shortfall = cap ? cap.mentorShortfall : 0;

    return [
      String(i + 1),
      s.config?.name || `Scenario ${i + 1}`,
      formatNumber(s.endorseRate, 1) + "%",
      formatCurrencyDisplay(s.wtpPerTraineePerMonth, 0),
      formatCurrencyDisplay(s.costs?.totalEconomicCostAllCohorts ?? 0, 0),
      formatCurrencyDisplay(s.epiBenefits?.totalEpiBenefitAllCohorts ?? 0, 0),
      formatCurrencyDisplay(s.netBenefitAllCohorts ?? 0, 0),
      s.bcrAllCohorts !== null && s.bcrAllCohorts !== undefined ? formatNumber(s.bcrAllCohorts, 2) : "-",
      formatNumber(mentorsReq, 0),
  ...
  const colWidths = [18, 190, 48, 55, 70, 70, 60, 30, 50, 52, 70];
  ...
  doc.setFont("helvetica", "normal");
  doc.setFontSize(9);
  const note = "Feasibility uses mentors required nationally vs available mentors (Settings ▸ Capacity and costs).";
  addWrappedText(doc, note, margin, y, pageW - 2 * margin, 9);
  y += 18;

  // Assumptions box (compact) on page 1 bottom if room; else page 2.
  ...
  if (y < pageH - margin - 120) {
    doc.setFont("helvetica", "bold");
    doc.setFontSize(12);
    doc.text("Assumptions used (summary)", margin, y);
    y += 8;
    y = pdfDrawTextBox(doc, { x: margin, y, width: pageW - 2 * margin, text: boxText, fontSize: 9, padding: 10 });
  } else {
    // Defer to page 2 if not enough space.
  }

  // Page 2: Enablers + Risks (+ assumptions if not printed)
  doc.addPage();
  let y2 = margin;

  doc.setFont("helvetica", "bold");
  doc.setFontSize(16);
  doc.text("Implementation enablers, risks, and feasibility notes", margin, y2);
  y2 += 12;

  const exportNotes = getExportNotesFromUI();
  const enablers = parseBulletsText(exportNotes.enablers || "");
  const risks = parseBulletsText(exportNotes.risks || "");

  doc.setFont("helvetica", "bold");
  doc.setFontSize(12);
  doc.text("Enablers", margin, y2);
  y2 += 8;
  doc.setFont("helvetica", "normal");
  doc.setFontSize(10);
  y2 = pdfDrawBullets(doc, enablers.length ? enablers : ["(No enablers entered in the tool.)"], margin, y2, pageW - 2 * margin);

  y2 += 8;
  doc.setFont("helvetica", "bold");
  doc.setFontSize(12);
  doc.text("Risks and mitigation focus", margin, y2);
  y2 += 8;
  doc.setFont("helvetica", "normal");
  doc.setFontSize(10);
  y2 = pdfDrawBullets(doc, risks.length ? risks : ["(No risks entered in the tool.)"], margin, y2, pageW - 2 * margin);

  // Add assumptions box if not printed on page 1
  y2 += 10;
  if (y > pageH - margin - 120) {
    doc.setFont("helvetica", "bold");
    doc.setFontSize(12);
    doc.text("Assumptions used (summary)", margin, y2);
    y2 += 8;
    y2 = pdfDrawTextBox(doc, { x: margin, y: y2, width: pageW - 2 * margin, text: boxText, fontSize: 9, padding: 10 });
  }

  addPdfPageNumbers(doc, margin);
  doc.save("steps_top5_scenarios_mini.pdf");
  showToast("Top 5 mini PDF downloaded (2 pages).", "success");
}
